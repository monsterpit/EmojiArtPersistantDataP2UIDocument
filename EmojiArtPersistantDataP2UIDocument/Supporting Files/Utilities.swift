...
/*
 All image fetcher does is it takes a URL goes and fetches the image make sure it is an Image
 and when it's sure its got an image it calls you back with an handler that you can give to it
 So its kinda a asynchronous check and make sure this is actually an image
 
 It also a cool feature where it has a backup image
 So if I go out and fetch the URL and get it back whoa that's not an URL of the image then it uses the backup image it creates a URL for that a local URl in the filesystem and then gives you back a URL so it's kind of a backup
 
 Because I wanna go fetch that image and if i can get it I will use that URL , but if I cant I want the image that was being dragged instead and I will create a local URL to it so that's all image fetcher does
